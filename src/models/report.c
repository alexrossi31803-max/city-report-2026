...
struct ReportStruct {
    ...
    char citizen_name[MAX_NAME + 1];  /* Array statico protetto per l'username (50 byte su file) */
    ReportCategory category;         /* Categoria dell'anomalia (1 byte su file) */
    char description[MAX_DESC + 1];   /* Array statico per il corpo descrittivo (256 byte su file) */
    char date[12];                    /* Array statico per la data "GG/MM/AAAA\0" (11 byte su file) */
...
Report create_report(
    ...
    strncpy(r->citizen_name, citizen_name, MAX_NAME);
    r->citizen_name[MAX_NAME] = '\0'; /* Sigillo di chiusura stringa obbligatorio in C */
