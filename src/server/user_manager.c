...
/**
 * @brief Espande la tabella hash allocando 50 nuovi slot vuoti per scongiurare la saturazione.
...
/**
 * @brief Calcola quanti utenti sono registrati dividendo la dimensione del file per il passo a 107 byte.
 */
...
bool register_user(const char* username, const char* password, UserRole role) {
    ...
    for (int i = 0; i < capacity; i++) {
        ...
        if (slot_value == -1) {
            ...
            fprintf(f_idx, "%05d", assigned_data_row);
            slot_found = true;
            break;
        }
        ...
        if (f_usr) {
            ...
            /* Salto geometrico nel database anagrafico a passo USER_LINE_TOTAL (107 byte) */
    ...
    /* Append finale dei dati anagrafici a 107 byte nel database principale */
