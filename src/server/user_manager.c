...
static bool expand_users_index() {
    ...
    if (!f) return false;
    for (int i = 0; i < BLOCK_SIZE_USERS; i++) fputs("-1   \n", f);
    fclose(f);
    return true;
}
...
bool register_user(const char* username, const char* password, UserRole role) {
    ...
    for (int i = 0; i < capacity; i++) {
        fseek(f_idx, current_index * USER_IDX_LINE, SEEK_SET); 
        ...
        if (valore_slot == -1) {
            ...
            fseek(f_idx, current_index * USER_IDX_LINE, SEEK_SET);
            fprintf(f_idx, "%05d", riga_dati_assegnata);
            slot_trovato = true;
            break;
        }
        ...
        if (f_usr) {
            ...
            if (fgets(user_line, sizeof(user_line), f_usr)) {
                int tmp_id; char tmp_user[MAX_USERNAME + 1]; char tmp_pass[MAX_PASSWORD + 1]; UserRole tmp_role;
                line_to_user_data(user_line, &tmp_id, tmp_user, tmp_pass, &tmp_role);
                if (strcmp(tmp_user, username) == 0) { fclose(f_usr); fclose(f_idx); return false; }
            }
            fclose(f_usr);
        }
    ...
    user_to_line(final_line, riga_dati_assegnata, username, password, role); 
...
User login_user(const char* username, const char* password) {
    ...
    for (int i = 0; i < capacity; i++) {
        ...
        if (f_usr) {
            ...
            fseek(f_usr, valore_slot * USER_LINE_TOTAL, SEEK_SET);
            if (fgets(user_line, sizeof(user_line), f_usr)) {
                int u_id; char u_user[MAX_USERNAME + 1]; char u_pass[MAX_PASSWORD + 1]; UserRole u_role;
                line_to_user_data(user_line, &u_id, u_user, u_pass, &u_role);
                if (strcmp(u_user, username) == 0) {
                    if (strcmp(u_pass, password) == 0) utente_autenticato = create_user(valore_slot, u_user, u_pass, u_role);
                    fclose(f_usr); break; 
                }
            }
            fclose(f_usr);
