...
/* Estrae l'indice riga libero dallo stack LIFO dei buchi del rispettivo file di stato */
static int pop_hole_index(const char* hole_path) {
    ...
    // Legge l'ultimo inserimento registrato in coda (Semantica LIFO Stack)
    ...
    // Troncamento sul posto dell'elemento rimosso dallo stack dei buchi
    long new_size = (long)(line_count - 1) * SYSTEM_REG_LINE;
#ifdef _WIN32
    #include <io.h>
    #include <fcntl.h>
    int fd = _open(hole_path, _O_RDWR);
...
/* Inserisce l'indice riga liberato all'interno dello stack LIFO di riferimento */
...
/* Modifica chirurgica ad accesso immediato del flag di cella ('A', 'N', 'E') nel file master */
static void set_master_cell_state(const char* path, int disk_row, char state) {
    if (disk_row < 0) return;
    FILE* f = fopen(path, "rb+");
    if (!f) return;
    
    fseek(f, (long)disk_row * REPORT_LINE_TOTAL, SEEK_SET);
    char line[REPORT_LINE_TOTAL + 1];
    if (fread(line, sizeof(char), REPORT_LINE_TOTAL, f) == REPORT_LINE_TOTAL) {
        line[350] = state;
        fseek(f, (long)disk_row * REPORT_LINE_TOTAL, SEEK_SET);
        fwrite(line, sizeof(char), REPORT_LINE_TOTAL, f);
    }
    fclose(f);
}
...
bool process_and_flush_bench_v2(void) {
    ...
    for (unsigned int i = 0; i < counter_bench; i++) {
        ...
        if (!r || cell_state != 'A') { if (r) free_report(r); continue; }
        ...
        if (f_master) {
            if (assigned_row == -1) { 
                ...
                assigned_row = (int)(ftell(f_master) / REPORT_LINE_TOTAL);
            }
            set_report_disk_row(r, assigned_row);
            
            char out_line[REPORT_LINE_TOTAL + 1];
            report_to_line(out_line, r, 'A');
            fseek(f_master, (long)assigned_row * REPORT_LINE_TOTAL, SEEK_SET);
            fwrite(out_line, sizeof(char), REPORT_LINE_TOTAL, f_master);
            fclose(f_master);
            
            // Aggiorna le statistiche in O(1) legate allo stato e alla categoria
            unsigned int st_count = read_system_variable(reg_idx_status);
            write_system_variable(reg_idx_status, st_count + 1);
            ...
            unsigned int cat_count = read_system_variable(cat_idx);
            write_system_variable(cat_idx, cat_count + 1);
        }
        free_report(r);
    }
    fclose(f_bench);
...
bool register_report_from_citizen_ram(Report r) {
    ...
    if (f_bench) {
        for (unsigned int i = 0; i < counter_bench; i++) {
            ...
            if (fread(line, sizeof(char), REPORT_LINE_TOTAL, f_bench) != REPORT_LINE_TOTAL) continue;
            char st;
            Report tmp = line_to_report_v2(line, &st);
        ...
        if (trovato_in_bench) { 
            char out_line[REPORT_LINE_TOTAL + 1];
            report_to_line(out_line, r, 'A');
            fseek(f_bench, (long)bench_slot_index * REPORT_LINE_TOTAL, SEEK_SET);
    ...
    if (found_old_row != -1) {
        ...
        // Decrementa i contatori dello vecchio stato rimosso per compensare lo sfoltimento master
        ...
        unsigned int total_reports = read_system_variable(REG_IDX_NM_REPORT);
        write_system_variable(REG_IDX_NM_REPORT, total_reports + 1);
    }
    ...
    if (f_bench) {
        char out_line[REPORT_LINE_TOTAL + 1];
        report_to_line(out_line, r, 'A');
        fseek(f_bench, (long)counter_bench * REPORT_LINE_TOTAL, SEEK_SET);
        fwrite(out_line, sizeof(char), REPORT_LINE_TOTAL, f_bench);
        fclose(f_bench);
