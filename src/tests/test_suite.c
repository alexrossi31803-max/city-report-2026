...
/**
 * @brief Valida le scomposizioni asimmetriche separate dei parser a REPORT_BENCH_LINE e REPORT_MASTER_LINE byte.
 */
static void test_dual_geometry_parsing_mechanisms(void) {
    printf("[TEST] Validazione Parser Asimmetrico (REPORT_BENCH_LINE Bench vs REPORT_MASTER_LINE Master)... ");
    
    Report r = create_report(555, 99, "UserTest", WASTE, "Rifiuti", "18/05/2026", '0');
    set_report_disk_row(r, 15);

    /* Test 1: Configurazione geometrica MASTER a REPORT_MASTER_LINE byte */
    char master_line[REPORT_MASTER_LINE + 1];
    report_to_line(master_line, r, 'A');
    assert(strlen(master_line) == REPORT_MASTER_LINE);
    assert(master_line[REPORT_MASTER_LINE-2] == 'A');
    assert(master_line[REPORT_MASTER_LINE-1] == '\n');

    /* Verifiche del parsing inverso di riga Master */
    char extracted_state;
    Report r_master = line_to_report(master_line, &extracted_state);
    ...
    /* Test 2: Configurazione geometrica BENCH a REPORT_BENCH_LINE byte */
    char bench_line[REPORT_BENCH_LINE + 1];
    report_to_line(bench_line, r, '\0');
    assert(strlen(bench_line) == REPORT_BENCH_LINE);
    assert(bench_line[REPORT_BENCH_LINE-1] == '\n'); /* Sulla BENCH il newline risiede direttamente al byte REPORT_BENCH_LINE-1 */
...
static void test_disk_binary_search_indices(void) {
    ...
    unsigned int *collected_results = NULL;
