...
void run_all_tests() {
    printf("\n===================================================\n");
    printf("     AVVIO TEST SUITE AUTOMATIZZATA (REQUISITI AVL)   \n");
    printf("===================================================\n");
...
void test_user_registration_and_login() {
    ...
    ASSERT_TRUE(wrong_pass == NULL, "Login desiderato rifiutato correttamente con password errata.");
...
void test_report_registration_and_ram_list() {
    ...
    Report r1 = create_report(9001, 101, "Mario", ROAD, "Buca profonda via Roma", "14/05/2026", '2');
...
void test_revert_stack_logic() {
    ...
    Report original = create_report(9500, 102, "Luigi", LIGHTING, "Lampione spento", "14/05/2026", '0');
    list_insert(ram_list, original);
    
    Report r_to_mod = list_find(ram_list, 9500);
    stack_push(rev_stack, r_to_mod);
    
    Report modified = create_report(9500, 102, "Luigi", LIGHTING, "Lampione spento e rotto", "14/05/2026", '1');
...
void test_report_state_update_and_file_transfer() {
    printf("\n[TEST] Avanzamento Stato e Geometria Riga Master a 352 Byte:\n");
    unsigned int test_rep_id = 8888;
    Report r_test = create_report(test_rep_id, 103, "StatoTester", WASTE, "Rifiuti ingombranti", "14/05/2026", '1');
    set_report_disk_row(r_test, 12); 
    ...
    char r_state;
    Report read_back = line_to_report_v2(buffer_line, &r_state);
    ...
    ASSERT_TRUE(final_check, "Mappatura inversa del record verificata sul parser a 352 byte.");
...
void test_priority_queue_sorting() {
    printf("\n[TEST] Ordinamento Coda a Priorità (Urgenza Scalare + Data FIFO):\n");
    PriorityQueue pq = create_pq();
    
    Report r1 = create_report(8001, 104, "UserA", ROAD, "Problema Medio", "12/05/2026", '1');
    Report r2 = create_report(8002, 104, "UserB", ROAD, "Problema Alto Recente", "13/05/2026", '2');
    Report r3 = create_report(8003, 104, "UserC", ROAD, "Problema Alto Vecchio", "10/05/2026", '2');
...
void test_report_avl_indexing() {
    printf("\n[TEST] Integrita' Albero Auto-Bilanciante AVL:\n");
    FILE* f_rep = fopen(PATH_AVL_REPORT_ID, "wb");
    ASSERT_TRUE(f_rep != NULL, "Verificata accessibilita' e scrittura del file d'indice AVL.");
    if (f_rep) fclose(f_rep);
}

void test_statistical_report_generation() {
    printf("\n[TEST] Integrità dei Canali di Database Master Storici:\n");
    FILE* f1 = fopen(PATH_OPEN_MASTER, "rb");
    FILE* f2 = fopen(PATH_PROGRESS_MASTER, "rb");
    FILE* f3 = fopen(PATH_CLOSED_MASTER, "rb");
    
    ASSERT_TRUE(f1 != NULL || f2 != NULL || f3 != NULL, "Accessibilita' binarizzata stazionaria verificata.");
